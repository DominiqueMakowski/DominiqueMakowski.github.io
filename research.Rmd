...
data <- data.frame(word = rownames(as.matrix(text_clean)), freq = rowSums(as.matrix(text_clean))) %>% 
  ...
  filter(freq >  50) %>% 
  filter(freq < Inf) %>% 
  filter(!word %in% c("dans", "plus", "pour", "avec", "cette", "comme", "sont", "tion", "also", "france", "ction", "nature"))
...
Tuena, C., Serino, S., Gaston-Bellegarde, A., Orriols, E., **Makowski, D.**, Riva, G. & Piolino, P. (2017). *How Virtual Embodiment Affects Episodic Memory Functioning: a Proof-of-Concept Study*, 27 June, Milano, Italy.
...
Sperduti, M., **Makowski, D.** & Piolino, P. (2016). *Does mindfulness enhances interoception? Dissociation between different interoceptive abilities*. 2nd International Conference on Mindfulness, 11-15May, Roma, Italy.
