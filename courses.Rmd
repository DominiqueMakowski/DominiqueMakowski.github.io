---
title: "Courses / Trainings"
bibliography: homepage.bib
---

```{r, out.width = "550px", echo = FALSE, eval = TRUE, fig.align='center'}
knitr::include_graphics("media/conf3.jpg")
```


My technical field of expertise lies in applied statistics with R, complex experiment creation, EEG and biosignal processing with Python.




## Neuropsychology

| Link          | Description |
|------------------------------|--------------------------------------|
| [Experiment Creation](http://neuropsydia.readthedocs.io/en/latest/tutorials/index.html)| Create experiments, tasks or questionnaires with Neuropsydia for Python.| 


## Neuroimaging

| Link          | Description |
|------------------------------|------------------------------------|
| [Biosignals processing in Python](http://neurokit.readthedocs.io/en/latest/tutorials/Bio.html)| Process biosignals, such as electrodermal (EDA), cardiac (ECG), respiratory (RSP) or muscular (EMG) data with Python and NeuroKit.
| [EEG](http://neurokit.readthedocs.io/en/latest/tutorials/EEG.html) | EEG and MEG processing using Python and NeuroKit (currently under developpment).|

## Statistics

| Link          | Description |
|------------------------------|--------------------------------------|
| [To Do](http://rpubs.com/YaRrr/MLTutorial)| The basics of R for psychology and neuroscience.| 


